...
static void showtime(uint8 time)
{
...
static void appLight(uint16 node)
{
    ...
    while(1){
      ...
      if(key == 4){
        ...
        prepare(now);
        for(int i=0;i<=5;++i) basicRfSendPacket(board_cast_table[i], pTxData, APP_PAYLOAD_LENGTH);//发送数据

        halIntOff();                                               //关闭全局中断

        halIntOn();                                               //打开中断
        
        while(1){  halMcuWaitMs(1000);
        now++;
        showtime(now);
